...
// $_SESSION['folder_name'] = $name = $_GET['name'];
// $_SESSION['targetPath'] = $getNum = $_GET['num'];
// $_SESSION['back'] = $back = $_GET['id'];
// $folderName = $_SESSION['folder_name'];
// $usersID = $_SESSION['usersID'];

// $folderID = $_SESSION['folderID'];
// $query = "SELECT name,area,letter FROM usersfolder WHERE usersFolderID = $folderID";
// $result = mysqli_query($con,$query);
// $rows = $result->fetch_assoc();
// $area = $rows['area'];

// $query2 = "SELECT areaName from areas WHERE id = $area";
// $result2 = $con->query($query2);
// $rows2 = $result2->fetch_assoc(); 

// $query = "SELECT files.id,files.fileName,files.path,tags.tagNum FROM tags,files WHERE targetPath = $back AND tags.fileName = files.id AND files.status = 'notArchived'";
// $res= mysqli_query($con, $query);
// $numRow = mysqli_num_rows($res);
// $count = 0;
// $numFile = array();
// while($count <= $numRow){
//   array_push($numFile,++$count);
// }
...
?>
    ...
    <div class="container-fluid">
          ...
          <ol class="breadcrumb mrgnTop">
            ...
            <li class="breadcrumb-item active"><strong>Area <?=$areaNum?> <?=$rows2['areaName']?> / <?=$rows['letter']?>. <?=$rows['name']?> / <?=$rows['letter']?>.<?=$folderNum?> <?=$subName?></strong></li>
          </ol>
      
        <div style="margin-bottom: 10px">
        <a href="<?=base_url()?>ADMS/areaFolderV/<?= $areaNum?>/<?= $usersFolderID?>" class="btn cstm-btn-red" ><i class="fa fa-arrow-left"></i> Back</a>
            ...
            <table class="table table-striped" id="dataTable" width="100%" cellspacing="0" >
                    ...
                    ?>       
                                ...
                                <td style="width:5%"><span 
                                        ...
                                        ?>
                                    ><?=$row['tagNum']?>.</span></td>
                                    <td><a href="<?=base_url()?>ADMS/viewV3/<?= $row['fileID'] ?>/<?=$areaNum?>/<?=$usersFolderID?>/<?=$folderNum?>/<?=$subFolderID?>?name=<?=$subName?>"><?= $row['fileName'] ?></a></td> 
...
<div id="deleteFile" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <h4 class="modal-title" style="text-align:center"><span>Are you sure you wanna delete this tagged file <i class="fa fa-trash"></i> ?</span></h4>
            ...
            <form method="post" action="deleteFileInTagFolder.php">
                ...
                <div class="control-group">
                 <div align="center" class="controls">                    
                      <button name="submit" type="submit" class="btn cstm-btn-navy"><i class="fa fa-check"></i> Yes</button>
                      <button  type="button" class="btn cstm-btn-red" data-dismiss="modal"><i class="fa fa-times"></i> No</button>
                  </div>
              </div>
            </form>
            </div>
...
$(document).ready(function(){
    ...
    $(document).on('change','#num',function(){
        ...
        $.ajax({  
                url:"numFile.php",  
                method:"post",  
                data:{value:value,fileID:fileID},  
